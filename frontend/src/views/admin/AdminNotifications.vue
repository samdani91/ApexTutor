<template>
  <div>
    ...
    <!-- Filters -->
    <div v-if="!loading" class="mb-5 flex flex-wrap items-center gap-3">
      <DropSelect
        v-model="sortOrder"
        :options="sortOptions"
        placeholder="Sort…"
      ...
      <span v-if="typeFilter" class="ml-auto text-xs text-paper-400 font-body">
    ...
    <div v-else-if="!filteredNotifications.length" class="card flex flex-col items-center justify-center py-20 text-center">
      ...
      <p class="text-paper-400 text-sm font-body mt-1 max-w-xs">Admin activity and alerts will appear here.</p>
    </div>
      ...
      <div v-for="n in filteredNotifications" :key="n.id"
        ...
        <div class="flex flex-col gap-4 sm:flex-row sm:items-start">
          ...
          <div class="flex-1 min-w-0">
            ...
            <!-- Shortlist: show guardian + tutor cards -->
            <div v-if="n.data.type === 'tutor_shortlisted'" class="mt-3 grid gap-2 sm:grid-cols-2">
              <div class="rounded-md bg-paper-50 px-3 py-2">
                ...
                <p class="text-xs text-paper-400 font-body">ID {{ n.data.guardian_id }}</p>
              </div>
              <div class="rounded-md bg-paper-50 px-3 py-2">
                ...
                <p class="text-xs text-paper-400 font-body">ID {{ n.data.tutor_id }}</p>
              </div>
            </div>

            <!-- Connection: show guardian + tutor + optional message -->
            ...
            <!-- Pending profile change: action link -->
            ...
            <!-- Pending video: action link to tutor detail -->
            <div v-else-if="n.data.type === 'pending_video'" class="mt-3">
              <RouterLink :to="`/admin/tutors/${n.data.tutor_profile_id}`"
...
const filteredNotifications = computed(() => {
  ...
  if (typeFilter.value) {
    list = list.filter(n => n.data.type === typeFilter.value)
  }
