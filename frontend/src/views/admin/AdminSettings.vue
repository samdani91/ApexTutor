<template>
  <div class="max-w-xl">
    <h1 class="font-display font-bold text-2xl text-navy-900 mb-6">Account settings</h1>

    <!-- Sub-tabs -->
    <div class="flex bg-paper-100 rounded-lg p-1 gap-1 mb-6 w-fit">
      <button
        v-for="tab in tabs" :key="tab.key"
        @click="activeTab = tab.key"
        class="flex items-center gap-2 px-4 py-1.5 rounded-md text-sm font-semibold font-display transition-colors"
        :class="activeTab === tab.key ? 'bg-white text-navy-900 shadow-sm' : 'text-paper-500 hover:text-navy-700'"
      >
    ...
    <div v-if="activeTab === 'profile'" class="card">
      <h2 class="font-display font-semibold text-navy-700 text-base mb-4 pb-2 border-b border-paper-100">
        Profile information
      </h2>
      ...
      <div class="flex items-center gap-4 mb-5">
        <div class="relative shrink-0">
          <div class="w-16 h-16 rounded-xl bg-navy-100 flex items-center justify-center overflow-hidden ring-2 ring-white shadow">
          ...
          <label class="absolute bottom-0 right-0 w-6 h-6 bg-navy-700 rounded-full flex items-center justify-center cursor-pointer hover:bg-navy-800 shadow transition-colors">
        ...
        <div>
          <p class="font-display font-semibold text-navy-900">{{ auth.user?.name }}</p>
      ...
      <form @submit.prevent="saveProfile" class="space-y-4">
        ...
        <div>
          ...
          <textarea v-model="profileForm.address" name="address" id="address" autocomplete="street-address"
            class="input resize-none" rows="2" placeholder="Your address…" maxlength="500" />
        </div>
        <p v-if="profileError" class="text-red-600 text-sm font-body">{{ profileError }}</p>
    ...
    <div v-if="activeTab === 'security'" class="card">
      ...
      <form @submit.prevent="savePassword" class="space-y-4">
        ...
        <p v-if="passwordError" class="text-red-600 text-sm font-body">{{ passwordError }}</p>
