      ...
      <nav class="flex-1 p-3 space-y-0.5 overflow-y-auto">
        ...
        >
          <svg class="w-[18px] h-[18px] shrink-0" fill="none" stroke="currentColor" stroke-width="1.75" viewBox="0 0 24 24">
            ...
            <path v-else-if="item.icon === 'support'" stroke-linecap="round" stroke-linejoin="round"
            ...
            <path v-else-if="item.icon === 'briefcase'" stroke-linecap="round" stroke-linejoin="round"
              d="M20.25 14.15v4.25a2.25 2.25 0 01-2.25 2.25h-12a2.25 2.25 0 01-2.25-2.25v-4.25m16.5 0a2.18 2.18 0 00.75-1.661V8.706c0-1.081-.768-2.015-1.837-2.175a48.114 48.114 0 00-3.413-.387m4.5 8.006c-.194.165-.42.295-.673.38A23.978 23.978 0 0112 15.75c-2.648 0-5.195-.429-7.577-1.22a2.016 2.016 0 01-.673-.38m0 0A2.18 2.18 0 013 12.489V8.706c0-1.081.768-2.015 1.837-2.175a48.111 48.111 0 013.413-.387m7.5 0V5.25A2.25 2.25 0 0013.5 3h-3a2.25 2.25 0 00-2.25 2.25v.894m7.5 0a48.667 48.667 0 00-7.5 0M12 12.75h.008v.008H12v-.008z"/>
            <path v-else-if="item.icon === 'clipboard'" stroke-linecap="round" stroke-linejoin="round"
          ...
          <span v-else-if="item.supportBadge && activeTicketCount > 0"
            class="ml-auto text-[10px] font-bold font-display bg-red-500 text-white rounded-full min-w-[18px] h-[18px] flex items-center justify-center px-1 shrink-0">
...
const navItems = computed(() => {
  if (auth.isTutor) return [
    ...
    { to: '/tutor/support',             label: 'Support',            icon: 'support', supportBadge: true },
  ]
  if (auth.isGuardian) return [
    ...
    { to: '/guardian/jobs',                 label: 'My Jobs',            icon: 'briefcase' },
    ...
    { to: '/guardian/support',              label: 'Support',            icon: 'support', supportBadge: true },
  ]
  return []
})
